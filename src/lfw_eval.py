...
def eval(model, model_path=None, is_gray=False):
    ...
    model.eval()
    root = '../data/lfw_aligned/'
    with open('../data/lfw-align-128/pairs.txt') as f:
...
def eval_roc(model, model_path=None, is_gray=False):
    predicts = []
    model.load_state_dict(torch.load(model_path))
    model.eval()
    root = '../data/lfw_aligned/'
    ...
    with torch.no_grad():
        for i in tqdm(range(6000)):
            ...
            distance = f1.dot(f2) / (f1.norm() * f2.norm() + 1e-5)
            predicts.append('{}\t{}\t{}\t{}\n'.format(name1, name2, distance, sameflag))
            y_true.append(sameflag)
            y_preds.append(distance)

    fpr, tpr, thre = roc_curve(y_true, y_preds)
    AUC = auc(fpr, tpr)
    print(AUC)
